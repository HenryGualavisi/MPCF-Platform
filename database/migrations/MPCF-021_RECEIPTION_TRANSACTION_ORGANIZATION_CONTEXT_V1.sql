-- ============================================================
-- MPCF-021 - RECEIPTION TRANSACTION ORGANIZATION CONTEXT V1
-- TRAZIX HG / MPCF Platform
--
-- Objetivo:
-- 1) Reemplazar la RPC de MPCF-019 sin modificar historicamente MPCF-019.
-- 2) Resolver organization_id desde profiles.organization_id usando auth.uid().
-- 3) Mantener supplier_organization_id exclusivamente como proveedor externo.
-- 4) Mantener las validaciones y la atomicidad de MPCF-019.
-- ============================================================
...
begin
  ...
  if v_lot is not null then
    insert into public.reception_agricultural_lots(
      ...
      v_reception_id,v_lot,v_lotkg,'Asociacion documental registrada desde recepcion operativa','CONFIRMADO'
    );
  end if;
...
select
  'MPCF-021',
  'RECEIPTION_TRANSACTION_ORGANIZATION_CONTEXT_V1',
  auth.uid(),
  'Resolucion server-side de organization_id desde profiles.organization_id para recepcion transaccional.'
where not exists (
  select 1 from public.schema_migrations where migration_code='MPCF-021'
);

select 'MPCF-021 PREPARED' as status;
